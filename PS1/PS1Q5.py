...
class Prob5():
    def __init__(self):
        ...
        ###### START CODE HERE ######
        self.cat = io.imread("cat.bmp")
        self.dog = io.imread("dog.bmp")
        ###### END CODE HERE ######
    ...
    def my_conv2d(self, image, filter):
        ...
        convolved_image[:,:,0] = np.clip(convolve(img[:,:,0], filter), 0, 1)
        convolved_image[:,:,1] = np.clip(convolve(img[:,:,1], filter), 0, 1)
        convolved_image[:,:,2] = np.clip(convolve(img[:,:,2], filter), 0, 1)
        ###### END CODE HERE ######
    ...
    def prob_5_1(self):
        ...
        identity_image = np.zeros(cat.shape)
        blur_image = np.zeros(cat.shape)
        sobel_image =  np.zeros(cat.shape)
        laplacian_image = np.zeros(cat.shape)

        identity_image = self.my_conv2d(cat, self.identity_filter)
        blur_image = self.my_conv2d(cat, self.blur_filter)
        sobel_image = self.my_conv2d(cat, self.sobel_filter)
        laplacian_image = self.my_conv2d(cat, self.laplacian_filter)
    ...
    def create_hybrid_image(self, image1, image2):
        ...
        hybrid_image = None
        ###### START CODE HERE ######
        img_1_lf = self.my_conv2d(image1, self.gaussian_filter)
        img_2_lf = self.my_conv2d(image2, self.gaussian_filter)

        img_2_hf = image2 / 255 - img_2_lf

        hybrid_image = np.clip(img_1_lf + img_2_hf, 0, 1)
