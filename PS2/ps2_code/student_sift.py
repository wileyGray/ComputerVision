...
def get_magnitudes_and_orientations(dx, dy):
    ...
    magnitudes   = np.power((np.power(dx,2) + np.power(dy,2)), (1/2))
...
def get_feat_vec(x,y,magnitudes, orientations,feature_width):
    ...
    NEAREST = 4
    RANGE   = (-np.pi, np.pi)
    SIZE    = int(np.floor(feature_width / NEAREST))
    arr     = np.zeros((NEAREST, NEAREST))

    for idx, val in np.ndenumerate(arr):

        y_ij        = int(np.floor((idx[0] - 2) * SIZE) + y)
        y_limit     = y_ij + SIZE

        x_ij        = int(np.floor((idx[1] - 2) * SIZE) + x)
        x_limit     = x_ij + SIZE
        #print(x_ij)
        #print(y_ij)  
        #print(x_limit)
        #print(y_limit)  
        mag         =   magnitudes[y_ij: y_limit, x_ij: x_limit]
        ori         = orientations[y_ij: y_limit, x_ij: x_limit]
        #print(mag)
        #print(ori)
        mag_flat    = np.ndarray.flatten(mag)
        ori_flat    = np.ndarray.flatten(ori)
        feature , _ = np.histogram(ori_flat, bins = BIN_NUM, range = RANGE, weights = mag_flat)
        fv          = np.append(fv, feature)
