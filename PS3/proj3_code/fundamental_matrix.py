...
def point_line_distance(line: np.ndarray, point: np.ndarray) -> float:
    ...
    ##############################
    p_e_dist  =         a * u + b * v + c
    pyth_dist = np.sqrt(a * a + b * b)
    
    error     = p_e_dist / pyth_dist
    ##############################
...
def signed_point_line_errors(x_0s: list,
    ...
    N = len(x_0s)
    for i in range(N):
        f_dot   = np.dot(F, x_1s[i])
        f_t_dot = np.dot(F.T, x_0s[i])
        d1      = point_line_distance(f_dot,   x_0s[i])
        errors.append(d1)
        d0      = point_line_distance(f_t_dot, x_1s[i])
        errors.append(d0)
        #print("d1: " + str(d1) + "d2: " + str(d2))
        #print("errors list recent: " + str(errors[:-2])  + str(errors[:-1]))
    ##############################
