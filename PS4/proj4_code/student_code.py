...
def nearest_neighbor_classify(train_image_feats,
    ...
    for distance in distances:
        ...
        for i in range(k):
            k_labels.append(train_labels[sort[i]])

        mode = stats.mode(k_labels, keepdims = True)[0][0]
        pred_labels.append(mode)
...
def kmeans(feature_vectors, k, max_iter=100):
    ...
    centroids   = feature_vectors[idxs]
    oldcentoids = None
    ...
    for iter in range(max_iter):
        ...
        for i in range(k):
            idxs = np.where(i == mins)
            fv   = feature_vectors[idxs] 
            l    = len(fv)
            if l == 0:
                break#optimizastion may need take
                #print("TAKEN")

            centroids[i] = np.mean(fv, axis = 0)
...
def build_vocabulary(image_arrays, vocab_size=50, stride=20, max_iter=10):
    ...
    *   The default vocab_size of 50 is sufficient for you to get a
    ...
    for i in range(len(image_arrays)):
        ...
        height     = img.shape[0]
        width      = img.shape[1]
        xv, yv     = generate_sample_points(height, width, stride)
        img_tensor = torch.from_numpy(img).view(1, 1, height, width)
        fvs        = get_siftnet_features(img_tensor, xv, yv)
        if fvs is None:
            print("none , error in buildvocab")
        else:
            for fv in fvs:
                final.append(fv)

    
    #final = np.ndarray(final)
    final = np.array(final)
    vocab = kmeans(final, vocab_size, max_iter)
...
def get_bags_of_sifts(image_arrays, vocabulary, stride=5):
    ...
    for i in range(num_images):
        ...
        torch_image = torch.Tensor(img).view(1, 1, height, width)

        progress = int((i * 100) / num_images)
        if progress > z:
            #print(str(progress) + " percent finished")
            z += 1

        torch_image = torch_image.type(torch.float32)
        fvs  = get_siftnet_features(torch_image, xv, yv)
        idxs = kmeans_quantize(fvs, vocab)
        h = np.histogram(idxs, bins = h_bins)[0]
